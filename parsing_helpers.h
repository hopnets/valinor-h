...
/*
 * parse_vhdr: parse the Valinor header and return the id
 */
static __always_inline __u32 parse_vhdr(struct hdr_cursor *nh,
					void *data_end,
					struct vhdr **vheader)
{
	int datalen;
	...
	*vheader = h;
	...
	if (datalen < 0) {
        // bpf_printk("negative datalen?!");
		return -1;
    }
    
	return bpf_ntohl(h->id);
}
